...
| Question | Tool |
|---|---|
| "Where is X defined?" / "Find symbol named X" | \`codegraph_search\` (use \`referencesType\` to find symbols referencing a type; use \`impl_for\` to find implementors of a trait/interface) |
| "What calls function Y?" | \`codegraph_callers\` (supports batch via \`symbols\` array) |
| "What does Y call?" | \`codegraph_callees\` |
| "How does X reach/become Y? / trace the flow from X to Y" | \`codegraph_trace\` (one call = the whole path, incl. callback/React/JSX dynamic hops) |
| "Where is this symbol used (any kind)?" | \`codegraph_usages\` (broader than callers — covers refs, type annotations, instantiations, pattern matches; use \`kind: "pattern_match"\` for match/if-let sites) |
| "What would break if I changed Z?" | \`codegraph_impact\` |
| "Show me Y's signature / source / docstring" | \`codegraph_node\` (supports batch via \`symbols\` array) |
| "Give me focused context for a task/area" | \`codegraph_context\` |
| "See several related symbols' source at once" | \`codegraph_explore\` (use \`path\` to filter by directory, \`strict\` to limit results to that directory) |
