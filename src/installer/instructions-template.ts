...
| 问题 | 工具 |
|---|---|
| "X 在哪里定义？" / "查找符号 X" | \`codegraph_search\`（\`referencesType\` 查引用某类型的所有符号，\`mutability\` 过滤借用模式；\`impl_for\` 查 trait/interface 的所有实现者；\`kind: "comment"\` 搜索注释；\`kind: "macro"\` 搜索宏调用） |
| "谁调用了 Y？" | \`codegraph_callers\`（支持 \`symbols\` 数组批量查询；加 \`kind\` 参数查非调用关系：\`"references"\`、\`"type_of"\`、\`"pattern_match"\`、\`"instantiates"\`、\`"all"\` 及框架特定边；\`include_external\` 显示项目外符号的引用） |
| "Y 调用了什么？" | \`codegraph_callees\`（\`include_external\` 显示对项目外符号的调用 — 第三方依赖、标准库宏等） |
