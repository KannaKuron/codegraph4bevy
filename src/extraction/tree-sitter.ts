...
export class TreeSitterExtractor {
  ...
  private visitNode(node: SyntaxNode): void {
    ...
    // in expression bodies (e.g. `Action<导航上>::new()`). Skips type_annotation
    // contexts — those are handled by extractTypeAnnotations in the
    // property/method/class extractors.
    else if ((nodeType === 'generic_type' || nodeType === 'scoped_type_identifier') && this.nodeStack.length > 0) {
      // Skip type-position contexts (type_annotation, parameter types, return types)
      // — extractTypeAnnotations already handles those.
      let parent = node.parent;
      let inTypePosition = parent?.type === 'type_annotation' || parent?.type === 'generic_type'
        || parent?.type === 'array_type' || parent?.type === 'union_type' || parent?.type === 'intersection_type'
        || parent?.type === 'parenthesized_type' || parent?.type === 'nullable_type'
        || parent?.type === 'optional_type' || parent?.type === 'type_arguments'
        || parent?.type === 'tuple_type' || parent?.type === 'lambda_type';
      // Walk up to check for type_annotation ancestor (e.g. generic_type inside intersection_type inside type_annotation)
      if (!inTypePosition) {
        let ancestor = parent;
        for (let i = 0; i < 5 && ancestor; i++) {
          if (ancestor.type === 'type_annotation') { inTypePosition = true; break; }
          ancestor = ancestor.parent;
        }
      }
      if (!inTypePosition && parent?.type !== 'impl_item') {
  ...
  private extractCall(node: SyntaxNode): void {
    ...
    // B13: Struct update syntax suppression — delegate to language hook.
    // Default: no suppression (all languages except Rust pass through).
    if (this.extractor?.shouldSuppressCall && this.extractor.shouldSuppressCall(node, calleeName, this.source)) {
      return;
    }
    ...
    if (node.type === 'type_identifier') {
      ...
      if (typeName && !this.BUILTIN_TYPES.has(typeName)) {
        ...
        this.unresolvedReferences.push({
          fromNodeId,
          referenceName: typeName,
          referenceKind: kind as typeof this.unresolvedReferences[number]['referenceKind'],
          line: node.startPosition.row + 1,
          column: node.startPosition.column,
        });
      }
