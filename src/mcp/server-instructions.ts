...
- **"符号 X 是什么？"** → \`codegraph_search\`（\`referencesType\` 查引用某类型的所有符号，\`mutability\` 过滤借用模式；\`impl_for\` 查 trait/interface 的所有实现者；\`kind: "comment"\` 搜索注释；\`kind: "macro"\` 搜索宏调用位置；支持正则如 \`/handle.*Event\$/\`）
